...
func (m *Model) View() string {
	...
	if m.dirListing {
		...
		return ui.Card("Git Repositories", sb.String(), ui.ColSecondary, cardWidth)
	}
	...
	if m.input.Active {
		...
		sb.WriteString("\n\n")
		if m.err != nil {
			sb.WriteString("  " + lipgloss.NewStyle().Foreground(ui.ColRed).Render("✗ "+m.err.Error()))
			sb.WriteString("\n\n")
		}
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColMuted).Render("  Enter confirm  ←→ cursor  Home/End  Esc cancel"))
		return ui.Card("Change Repository", sb.String(), ui.ColSecondary, cardWidth)
	}
