...
func (m *Model) View() string {
	...
	if m.dirListing {
		...
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColSecondary).Render("  📂 " + m.dirPath))
		sb.WriteString("\n\n")
		maxShow := m.height - 10
		if maxShow < 5 {
			maxShow = 5
		}
		highlightStyle := lipgloss.NewStyle().Foreground(ui.ColAccent)
		listContent := m.dirList.RenderWithFormat(maxShow, func(item string, selected bool) string {
			if selected {
				return highlightStyle.Render("  > " + item)
			}
			return lipgloss.NewStyle().Foreground(ui.ColText).Render("    " + item)
		})
		sb.WriteString(listContent)
		sb.WriteString("\n\n")
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColMuted).Render("  ↑↓ select  Enter open  Esc back"))
		if m.errMsg != "" {
			sb.WriteString("\n  " + lipgloss.NewStyle().Foreground(ui.ColRed).Render(m.errMsg))
		}
		return ui.Card("Log Files", sb.String(), ui.ColSecondary, cardWidth)
	}

	// 路径输入模式
	if m.input.Active {
		var sb strings.Builder
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColText).Render("  " + m.input.Prompt))
		sb.WriteString("\n")
		before := ui.RuneSubstr(m.input.Value, 0, m.input.Cursor)
		after := ui.RuneSubstr(m.input.Value, m.input.Cursor, ui.RuneLen(m.input.Value))
		inputLine := "  > " + before + lipgloss.NewStyle().Foreground(ui.ColAccent).Render("|") + after
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColText).Render(inputLine))
		sb.WriteString("\n\n")
		if m.errMsg != "" {
			sb.WriteString("  " + lipgloss.NewStyle().Foreground(ui.ColRed).Render("✗ "+m.errMsg))
			sb.WriteString("\n\n")
		}
		sb.WriteString(lipgloss.NewStyle().Foreground(ui.ColMuted).Render("  Enter confirm  ←→ cursor  Home/End  Esc cancel"))
		return ui.Card("Open Log", sb.String(), ui.ColSecondary, cardWidth)
	}
